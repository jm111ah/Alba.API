...
namespace AlbaAPI.App.Controllers
{
    ...
    public class ValuesController : ApiController
    {
        ...
        {
            _logger.Info("GET api/values 요청 수신 (App)");
            var result = await _sampleService.GetAllAsync();
            var resultList = result.ToList();
            _logger.Info($"GET api/values 응답 (App): {resultList.Count}개 항목");
        ...
        {
            _logger.Info($"GET api/values/{id} 요청 수신 (App)");
            var result = await _sampleService.GetByIdAsync(id);
            
            if (result == null)
            {
                _logger.Warning($"GET api/values/{id} 응답 (App): 항목을 찾을 수 없음");
                throw new ApiException($"ID {id}에 해당하는 항목을 찾을 수 없습니다.", "NOT_FOUND", 404);
            }
            
            _logger.Info($"GET api/values/{id} 응답 (App): id={result.Id}, name={result.Name}");
